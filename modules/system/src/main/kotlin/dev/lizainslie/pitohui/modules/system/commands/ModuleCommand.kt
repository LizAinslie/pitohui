...
fun buildModuleDescription(
    ...
    if (context.isInCommunity) {
        description +=
            "\n${if (supportsMarkdown) "**" else ""}Enabled:${if (supportsMarkdown) "**" else ""} ${if (module.isEnabledForCommunity(
                    context.communityId!!,
                )
            ) {
                "yes"
...
suspend fun shouldListModule(
    module: AbstractModule,
    context: CommandContext,
) = when (module.visibility) {
    ModuleVisibility.DEVELOPER -> {
        false
    }
    ...
    ModuleVisibility.MODERATOR -> {
        when (context) {
            is DiscordCommandContext -> context.checkCallerPermission(Permission.ManageGuild)

            // todo: change this to a more robust role-based system
            else -> false
        }
    }
}
...
val ModuleCommand =
    defineCommand("module", "Manage modules in your community") {
        platform(Discord) {
        ...
        subCommand("list", "List all modules") {
            handle {
                val modules =
                    bot.modules.loadedModules.filter {
                ...
                if (this is DiscordCommandContext) {
                    respond {
                        ...
                        description =
                            "Below are modules that are available on this bot, their descriptions, and whether or not they're enabled."
        ...
        subCommand("enable", "Enable a module") {
            ...
            handle {
                ...
                val module =
                    bot.modules.loadedModules.firstOrNull { it.name == moduleName.lowercase() } ?: run {
