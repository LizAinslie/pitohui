...
val ConfigCommand = defineCommand("config", "Manage loaded configurations") {
    platform(Discord)

    subCommand("reload", "Reload a configuration") {
        val configKey = argument("key", "The key of the configuration", ArgumentTypes.STRING) {
            required = true
        }
        ...
        handle {
            ...
            respond("""Loaded configuration `${config.key}`, new value:
                |```json
                |${config.jsonValue}
                |```""".trimMargin())
        }
    ...
    subCommand("peek", "View the contents of a config") {
        val configKey = argument("key", "The identifier of the configuration", ArgumentTypes.STRING) {
            required = true
        }
        ...
        handle {
            ...
            respond("""The current value of config `${config.key}` is:
                |```json
                |${config.jsonValue}
                |```""".trimMargin())
        }
