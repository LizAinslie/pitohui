...
val StarboardCommand =
    defineCommand(
        ...
        subCommand("create", "Create a starboard in this community.") {
            val channelArg =
                argument("channel", "The channel to use as the starboard.", ArgumentTypes.CHANNEL) {
                    required = true
                }

            val thresholdArg =
                argument("threshold", "The number of stars required to post a message to the starboard.", ArgumentTypes.INT) {
                    defaultValue = 5
                }

            val emojiArg =
                argument("emoji", "The emoji to use for starring messages.", ArgumentTypes.STRING) {
                    defaultValue = "⭐️"
                }

            val selfStarArg =
                argument("self_star", "Whether to allow users to star their own messages.", ArgumentTypes.BOOLEAN) {
                    defaultValue = false
                }
            ...
            handle {
                ...
                enforceDiscordType<DiscordCommandContext> {
                    enforceGuild { guild ->
                        ...
                        response.createFollowup(
                            "Starboard created successfully in channel ${discordChannel.mention} with threshold $threshold, emoji $emojiRep, self-star allowed: $selfStar.",
                        )
                    }
                }
            }
        ...
        subCommand("remove", "Remove the starboard from this community.") {
            val channelArg =
                argument("channel", "The starboard channel to remove.", ArgumentTypes.CHANNEL) {
                    required = true
                }
